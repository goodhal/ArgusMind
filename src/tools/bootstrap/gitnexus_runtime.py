...
def check_gitnexus_command() -> Tuple[bool, str]:
    """检查 gitnexus 命令是否存在且可执行。"""
    return _tool_version_ok("gitnexus")


def _install_with_cn_registry(npm_path: str) -> Tuple[bool, str]:
    r = _run(
        [npm_path, "i", "-g", "gitnexus", "--registry", _CN_NPM_REGISTRY],
        timeout=600.0,
    )
    out = (r.stdout or r.stderr or "").strip()
    if r.returncode == 0:
        return True, f"已使用国内 npm 源（{_CN_NPM_REGISTRY}）安装 gitnexus。"
    return False, f"国内 npm 源安装失败 (code={r.returncode}): {out[:500]}"


def _install_with_default_registry(npm_path: str) -> Tuple[bool, str]:
    r = _run([npm_path, "i", "-g", "gitnexus"], timeout=600.0)
    out = (r.stdout or r.stderr or "").strip()
    if r.returncode == 0:
        return True, "已使用默认 npm 源安装 gitnexus。"
    return False, f"默认 npm 源安装失败 (code={r.returncode}): {out[:500]}"


def ensure_gitnexus_command() -> Tuple[bool, str]:
    """
    确保 gitnexus 命令可用：
    1) 已存在则直接返回；
    ...
    3) 国内源失败则自动切换默认源重试。
    """
    ok, msg = check_gitnexus_command()
    if ok:
        return True, msg

    npm_path = shutil.which("npm")
    if not npm_path:
        return False, f"{msg}；未找到 `npm`，无法自动安装 gitnexus。"
    ...
    ok2, msg2 = check_gitnexus_command()
    if ok2:
